...
export const TrustHub = () => {
  ...
  const pillarColors = ["text-jvto-orange", "text-jvto-lime", "text-blue-500", "text-amber-500", "text-emerald-600", "text-jvto-navy"];
      ...
      <div className="container-authority">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-16 gap-8">
          <div className="max-w-2xl">
            <div className="badge-eyebrow bg-jvto-navy text-white mb-6">
              <Lock className="w-3 h-3" /> System Integrity Dashboard
            </div>
            <h2 className="heading-section mb-6">
              OPERATIONAL <br />
              <span className="text-jvto-orange">TRANSPARENCY.</span>
            </h2>
            ...
            {mainTabs.map((tab) => (
              <button
                ...
                className={`flex-1 lg:flex-none flex items-center justify-center gap-3 px-8 py-4 rounded-md font-black uppercase tracking-[0.2em] text-[10px] md:text-xs transition-all ${
                  activeTab === tab.id 
                    ? 'bg-jvto-navy text-white shadow-hover shadow-jvto-navy/20' 
                    : 'text-slate-400 hover:text-jvto-navy hover:bg-white'
                }`}
              >
        ...
        <div className="relative min-h-[600px]">
          <AnimatePresence mode="wait">
              ...
              >
                {pillars.map((item, i) => (
                  <div 
                    ...
                    className={`p-10 md:p-12 rounded-md border border-slate-100 bg-white shadow-card hover:shadow-hover hover:border-jvto-orange/30 transition-all group cursor-pointer flex flex-col justify-between relative overflow-hidden ${i === 0 ? 'md:col-span-2' : ''}`}
                  >
                    <div className="absolute top-0 right-0 w-40 h-40 bg-slate-50 rounded-bl-full -z-10 group-hover:bg-jvto-orange/5 transition-colors"></div>
                    <div>
                      <div className="flex justify-between items-start mb-12">
                        <div className="p-5 rounded-md bg-slate-50 group-hover:bg-white transition-colors border border-slate-100 group-hover:border-jvto-orange/20">
                      ...
                      <h3 className="text-2xl md:text-4xl font-black mb-6 uppercase leading-none tracking-tight text-jvto-navy group-hover:text-jvto-orange transition-colors">{item.title}</h3>
                    ...
                    <div className="flex items-center justify-between mt-12 pt-10 border-t border-slate-100">
                      ...
                      <ChevronRight className="w-6 h-6 text-slate-400 group-hover:text-jvto-orange group-hover:translate-x-2 transition-all" />
                    </div>
                  </div>
                ))}
              </motion.div>
            ...
            {activeTab === 'partners' && partnerHighlight && (
                ...
                exit={{ opacity: 0, y: -20 }}
                transition={{ duration: 0.4 }}
                className="rounded-md bg-jvto-navy text-white overflow-hidden border border-white/10 shadow-hover grid grid-cols-1 lg:grid-cols-2"
              >
                <div className="p-8 md:p-16 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-white/10 relative">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-8">
                      <div className="p-3 rounded-md bg-jvto-orange/20 border border-jvto-orange/30">
                        <Handshake className="w-6 h-6 text-jvto-orange" />
                      </div>
                      <span className="font-mono text-xs uppercase tracking-[0.4em] font-black text-jvto-orange">Strategic Affiliations</span>
                    </div>
                    <h2 className="heading-section text-white mb-8">
                      {partnerHighlight.title.split(' ').slice(0, 2).join(' ')} <br />
                      <span className="text-jvto-orange">{partnerHighlight.title.split(' ').slice(2).join(' ')}</span>
                    </h2>
                    <p className="text-lg md:text-xl font-light text-white/70 leading-relaxed mb-10 border-l-2 border-jvto-orange/30 pl-8">
                      {partnerHighlight.narrative}
                    </p>
                    <button 
                      onClick={() => navigate(partnerHighlight.link)}
                      className="px-8 py-4 bg-jvto-orange hover:bg-jvto-orange/90 text-white rounded-sm font-black uppercase tracking-widest transition-all flex items-center gap-3 group"
                    >
                ...
                <div className="p-8 md:p-16 bg-black/20 backdrop-blur-sm grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
                  {partnerHighlight.partners?.map((partner: any, idx: number) => (
                    <div key={idx} className="bg-white rounded-md p-8 flex flex-col items-center justify-center gap-4 shadow-card group/partner hover:scale-105 transition-all duration-500">
                      ...
                      <span className="font-mono text-[9px] uppercase tracking-widest font-black text-slate-400 group-hover/partner:text-jvto-orange">{partner.name}</span>
                    </div>
                  ))}
                  <div className="bg-jvto-orange/10 border border-jvto-orange/30 rounded-md p-8 flex flex-col items-center text-center gap-2">
                    <Globe className="w-8 h-8 text-jvto-orange mb-2" />
                  ...
                  {(['history', 'press', 'forensic'] as EvidenceSubTab[]).map((sub) => (
                    <button
                      ...
                      className={`font-mono text-[10px] md:text-xs uppercase tracking-[0.2em] font-bold pb-2 border-b-2 transition-all ${
                        evidenceTab === sub ? 'border-jvto-orange text-jvto-navy' : 'border-transparent text-slate-400 hover:text-slate-600'
                      }`}
                    >
                    ...
                    {evidenceTab === 'history' && (
                      ...
                      >
                          ...
                          {artifacts.map((art, i) => (
                            <div key={i} className="p-8 rounded-md bg-white border border-slate-200 shadow-card">
                              <div className="flex items-center gap-4 mb-6">
                                <div className="w-12 h-12 rounded-full bg-jvto-navy text-white flex items-center justify-center font-black">{art.year}</div>
                                <span className="font-mono text-[10px] uppercase tracking-widest text-slate-400">{art.type}</span>
                              </div>
                              <h4 className="text-xl font-black text-jvto-navy uppercase mb-2">{art.title}</h4>
                        ...
                        <div className="rounded-md overflow-hidden border border-slate-200 relative group">
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-jvto-navy/80 to-transparent flex items-end p-8">
                      ...
                      >
                          ...
                          {SSOT.press.map((article, idx) => (
                            <button
                              ...
                              className={`w-full text-left p-4 rounded-sm border transition-all ${
                                activePressIndex === idx ? 'bg-jvto-navy text-white border-jvto-navy shadow-card' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
                              }`}
                            >
                        ...
                        <div className="lg:w-2/3 space-y-6">
                          <div className="p-8 rounded-md bg-slate-50 border border-slate-200 relative">
                            ...
                            <p className="text-jvto-navy italic text-lg md:text-xl leading-relaxed mb-6 font-light relative z-10">"{activeArticle.quote}"</p>
                            <div className="flex items-center gap-3">
                              <div className="h-px w-8 bg-jvto-orange"></div>
                              <span className="font-mono font-bold text-jvto-orange uppercase tracking-widest text-[10px]">— {activeArticle.author}</span>
                            </div>
                          </div>
                          <a href={activeArticle.url} target="_blank" rel="noopener noreferrer" className="block relative rounded-md overflow-hidden border border-slate-200 group">
                            ...
                            <div className="absolute inset-0 bg-jvto-navy/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                              <div className="bg-white text-jvto-navy px-6 py-3 rounded-sm font-black uppercase tracking-wider text-[10px] flex items-center gap-2">Read Original <ExternalLink className="w-4 h-4" /></div>
                            </div>
                          </a>
                        </div>
                      </motion.div>
                    ...
                    {evidenceTab === 'forensic' && (
                      ...
                      >
                        {forensicDocs.map((doc, i) => (
                          <div key={i} className="p-6 rounded-md bg-white border border-slate-200 group hover:border-jvto-orange/30 transition-all shadow-card">
                            <div className="relative aspect-square rounded-sm overflow-hidden mb-6 bg-slate-100">
                              ...
                              <div className="absolute top-4 right-4 bg-jvto-navy/80 text-white px-2 py-1 rounded-sm text-[8px] font-mono uppercase tracking-widest">{doc.type}</div>
                            </div>
                            <h4 className="font-black text-jvto-navy uppercase mb-4">{doc.title}</h4>
                            <div 
                        ...
                        <div className="p-8 rounded-md bg-jvto-navy text-white flex flex-col justify-center items-center text-center gap-6 shadow-card">
                          <Lock className="w-12 h-12 text-jvto-orange" />
                          <h4 className="text-xl font-black uppercase tracking-tight">Full Audit Library</h4>
                          <button onClick={() => navigate('/verify-jvto')} className="w-full py-4 bg-jvto-orange text-white rounded-sm font-black uppercase tracking-widest text-[10px] hover:scale-105 transition-transform">Enter Vault</button>
                        </div>
                      </motion.div>
                    )}
