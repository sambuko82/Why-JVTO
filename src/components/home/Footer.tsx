...
export const Footer = () => {
  ...
  return (
    <footer className="bg-jvto-navy text-white pt-24 md:pt-32 pb-12 md:pb-20 overflow-hidden relative border-t border-white/5">
      {/* Grid Pattern */}
      ...
      <div className="container-authority relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-12 mb-24 md:mb-32">
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-slate-400 font-black uppercase tracking-tight">
              <li><button onClick={() => navigate('/tours')} className="hover:text-jvto-orange transition-colors">All Tours</button></li>
              <li><button onClick={() => navigate('/tours/from-surabaya')} className="hover:text-jvto-orange transition-colors">From Surabaya</button></li>
              <li><button onClick={() => navigate('/tours/from-bali')} className="hover:text-jvto-orange transition-colors">From Bali</button></li>
            </ul>
          </div>
          <div>
            <h4 className="text-xs font-black uppercase tracking-[0.2em] text-white/40 mb-8">Learn</h4>
            <ul className="space-y-4 text-sm text-slate-400 font-black uppercase tracking-tight">
              <li><button onClick={() => navigate('/why-jvto')} className="hover:text-jvto-orange transition-colors">Why JVTO</button></li>
              <li><button onClick={() => navigate('/why-jvto/our-team')} className="hover:text-jvto-orange transition-colors">Meet the Team</button></li>
              <li><button onClick={() => navigate('/why-jvto/reviews')} className="hover:text-jvto-orange transition-colors">Reviews</button></li>
              <li><button onClick={() => navigate('/travel-guide')} className="hover:text-jvto-orange transition-colors">Travel Guide</button></li>
            </ul>
          </div>
          <div>
            <h4 className="text-xs font-black uppercase tracking-[0.2em] text-white/40 mb-8">Verify</h4>
            <ul className="space-y-4 text-sm text-slate-400 font-black uppercase tracking-tight">
              <li><button onClick={() => navigate('/verify-jvto')} className="hover:text-jvto-orange transition-colors">Verify JVTO</button></li>
              <li><button onClick={() => navigate('/policy/booking-payment-cancellation')} className="hover:text-jvto-orange transition-colors">Booking Policy</button></li>
              <li><button onClick={() => navigate('/policy/inclusions-exclusions')} className="hover:text-jvto-orange transition-colors">Inclusions</button></li>
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-4 text-sm text-slate-400 font-black uppercase tracking-tight">
              {org.same_as_urls.map((url, i) => (
                <li key={i}><a href={url} target="_blank" rel="noopener noreferrer" className="hover:text-jvto-orange transition-colors">{url.split('/')[2]}</a></li>
              ))}
            </ul>
          </div>
        </div>
        ...
        <div className="pt-12 md:pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-10">
          <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10">
            <div className="flex items-center gap-3 px-4 py-2 bg-white/5 rounded-md border border-white/10">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60">Verified Operator: NIB 1102230032918</span>
            </div>
            <div className="flex items-center gap-3 px-4 py-2 bg-white/5 rounded-md border border-white/10">
              <span className="text-jvto-gold font-black text-xs">4.8★ Trustpilot</span>
            </div>
            <p className="text-[10px] md:text-xs text-slate-500 font-mono uppercase tracking-[0.2em] text-center md:text-left">
              © 2015–2026 {org.legalName} · Bondowoso 68214, East Java · License No. 1102230032918
            </p>
          </div>
