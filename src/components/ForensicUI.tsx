...
export const EvidenceBadge = ({ type, text }: { type: 'verified' | 'public' | 'doc', text: string }) => {
  const styles = {
    ...
    doc: "bg-white/10 text-slate-400 border-white/20"
  };
  
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-mono font-bold uppercase tracking-widest border ${styles[type]}`}>
...
export const SectionHeading = ({ title, subtitle, badge }: { title: string, subtitle?: string, badge?: string }) => (
  <div className="mb-16 text-center max-w-3xl mx-auto">
    ...
    {subtitle && <p className="text-slate-400 text-xl leading-tight font-light">{subtitle}</p>}
  </div>
);
...
export const HashBadge = ({ hash }: { hash: string }) => (
  <div className="bg-black/20 p-3 rounded-xl border border-white/5 flex items-center gap-3 group">
    ...
    <span className="font-mono text-[10px] text-slate-400 uppercase tracking-widest truncate">SHA-256: {hash}</span>
  </div>
);
...
export const PillarCard = ({ 
  ...
  <div className="bento-card bg-white/5 border-white/10 p-10 group hover:bg-white/10 transition-all relative overflow-hidden">
    ...
    <p className="text-slate-400 mb-8 leading-tight text-lg font-light italic border-l-2 border-safety-orange pl-6">
      "{claim}"
    </p>
    
    <div className="space-y-4 mb-10">
      <p className="font-mono text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Verifiable Evidence:</p>
