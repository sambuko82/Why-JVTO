    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-0 md:p-12">
        <motion.div 
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="absolute inset-0 bg-jvto-navy/95 backdrop-blur-xl"
          onClick={onClose}
        />
        ...
        >
          ...
          <div className="p-8 md:p-10 border-b border-slate-100 flex items-center justify-between bg-white/80 backdrop-blur-md z-20">
            <div className="flex items-center gap-6">
              <div className="w-14 h-14 md:w-16 md:h-16 bg-slate-50 rounded-md flex items-center justify-center text-jvto-navy shadow-card">
                {assetType === 'pdf' ? <FileText className="w-6 h-6 md:w-8 md:h-8" /> : <Search className="w-6 h-6 md:w-8 md:h-8" />}
              </div>
              <div>
                <h3 className="text-xl md:text-3xl font-black text-jvto-navy uppercase leading-none mb-2 tracking-tight">{assetTitle}</h3>
            ...
            <div className="flex items-center gap-6">
              <div className="hidden md:flex items-center gap-3 px-5 py-2.5 bg-jvto-lime/10 border border-jvto-lime/20 rounded-md text-jvto-lime font-black text-[10px] uppercase tracking-[0.2em] shadow-card">
                <ShieldCheck className="w-4 h-4" /> Integrity_Verified
              </div>
              <button 
                onClick={onClose}
                className="w-12 h-12 md:w-14 md:h-14 bg-slate-50 hover:bg-jvto-orange hover:text-white rounded-md transition-all duration-500 group flex items-center justify-center shadow-card"
              >
          ...
          <div className="flex-1 overflow-hidden relative bg-slate-50 flex items-center justify-center p-4 md:p-8">
            ...
            >
                  ...
                  <button
                    ...
                    className={`w-6 h-6 md:w-8 md:h-8 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                      activeAnnotation === anno.id 
                        ? 'bg-jvto-orange border-white scale-125 shadow-[0_0_20px_rgba(242,125,38,0.6)]' 
                        : 'bg-jvto-navy/80 border-jvto-orange hover:scale-110'
                    }`}
                  >
                    <Info className={`w-3 h-3 md:w-4 md:h-4 ${activeAnnotation === anno.id ? 'text-white' : 'text-jvto-orange'}`} />
                  </button>
                  ...
                  <AnimatePresence>
                    {activeAnnotation === anno.id && (
                      <motion.div
                        ...
                        className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-48 md:w-64 bg-jvto-navy text-white p-4 rounded-md border border-white/10 shadow-hover z-50 pointer-events-none"
                      >
                        <div className="scanline opacity-20"></div>
                        <div className="font-mono text-[11px] text-jvto-orange uppercase tracking-widest mb-1 font-black">
                          Forensic_Insight
                        </div>
                        ...
                        <div className="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[8px] border-t-jvto-navy"></div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
              ...
              {isLoupeActive && assetType === 'image' && (
                <div 
                  className="absolute w-32 h-32 md:w-48 md:h-48 rounded-full border-4 border-jvto-orange shadow-2xl pointer-events-none overflow-hidden z-30 bg-white"
                  style={{
                    left: `${mousePos.x}%`,
                    top: `${mousePos.y}%`,
                ...
                >
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-full h-[1px] bg-jvto-orange/30"></div>
                    <div className="h-full w-[1px] bg-jvto-orange/30 absolute"></div>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Controls Overlay */}
            <div className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-2 md:gap-4 bg-jvto-navy/90 backdrop-blur-md px-4 md:px-6 py-2 md:py-3 rounded-md md:rounded-md border border-white/10 z-40">
              <button onClick={() => setZoom(prev => Math.max(0.5, prev - 0.25))} className="p-2 text-white hover:text-jvto-orange transition-colors">
              ...
              <button onClick={() => setZoom(prev => Math.min(3, prev + 0.25))} className="p-2 text-white hover:text-jvto-orange transition-colors">
                <ZoomIn className="w-4 h-4 md:w-5 md:h-5" />
              </button>
              <div className="w-[1px] h-6 bg-white/10 mx-1 md:mx-2"></div>
              <button className="flex items-center gap-2 text-white hover:text-jvto-lime transition-colors font-mono text-[11px] font-bold uppercase tracking-widest">
          ...
          <div className="p-4 md:p-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-4 md:gap-6">
              <div className="flex flex-col">
                ...
                <span className="font-mono text-[11px] text-jvto-navy font-bold">ID_JKT_01</span>
              </div>
              <div className="hidden sm:flex flex-col">
                <span className="font-mono text-[11px] text-slate-500 uppercase tracking-widest mb-1">Timestamp</span>
                <span className="font-mono text-[11px] text-jvto-navy font-bold">{new Date().toISOString().split('T')[0]}</span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="status-live"></div>
              <span className="font-mono text-[11px] text-jvto-orange font-black uppercase tracking-widest">Live_Forensic_Session</span>
            </div>
          </div>
        </motion.div>
      </div>
