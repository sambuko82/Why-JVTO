      ...
      <div className="container mx-auto px-6 py-12 md:py-24 max-w-6xl relative z-10">
        ...
        >
          <div className="flex items-center gap-2 mb-6">
            <BookOpen className="w-4 h-4 text-jvto-orange" />
          ...
          {guideCategories.map((category, catIdx) => (
            <div key={category.id}>
              <div className="mb-8 md:mb-12">
                <h2 className="text-2xl md:text-3xl font-black text-jvto-navy uppercase tracking-tight mb-4">{category.title}</h2>
              ...
              <div className="space-y-4">
                {category.items.map((item, itemIdx) => (
                  <motion.div
                    ...
                    className="bento-card bg-audit-white p-6 md:p-8 group cursor-pointer hover:border-jvto-orange transition-all flex items-center justify-between"
                  >
                    <div className="flex items-center gap-4 md:gap-6">
                      ...
                      <div>
                        <h3 className="text-lg md:text-xl font-black text-jvto-navy uppercase leading-none mb-2 tracking-tight">{item.title}</h3>
                        <p className="text-slate-500 text-xs md:text-sm font-light leading-tight">{item.desc}</p>
                      </div>
                    </div>
                    <ChevronRight className="w-5 h-5 text-slate-300 group-hover:text-jvto-orange group-hover:translate-x-1 transition-all shrink-0" />
                  </motion.div>
                ))}
              </div>
            </div>
        ...
        <motion.div 
          ...
          className="mt-16 md:mt-24 bento-card bg-jvto-navy text-white p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 group cursor-pointer"
        >
          <div className="flex flex-col md:flex-row items-center text-center md:text-left gap-6 md:gap-8">
            <div className="w-16 h-16 rounded-2xl bg-white/5 text-jvto-orange flex items-center justify-center border border-white/10 group-hover:scale-110 transition-transform shrink-0">
          ...
          <div className="flex items-center gap-3 text-jvto-orange font-mono text-[11px] font-bold uppercase tracking-widest">
